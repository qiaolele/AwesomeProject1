...
class ListViewFetchCom extends Component{
    ...
    constructor(props){
        ...
        this.state = {
            ...
            dataSource: new ListView.DataSource({
                // 数据源格式自定义时候，需要手动实现这两个方法——自己定义的两个方法
                getSectionData: getSectionData,
                getRowData: getRowData,
                //判断是否进行每行数据刷新
                rowHasChanged: (row1,row2) => row1 !== row2,
                // 列表分段标题时需要实现的
                sectionHeaderHasChanged: (s1,s2) => s1 !== s2//控制片头标题
    ...
    fetchData() {
        ...
        for (i = 0; i < length; i++) {
            ...
            organization = organizations[i];//得到每一个title
